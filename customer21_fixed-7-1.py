...
class ExitOverlay(FloatLayout):
    def __init__(self, on_confirm, on_cancel, **kw):
        ...
        box.add_widget(Label(text="Exit App?", font_size=sp(17), bold=True, color=C_TEXT, size_hint_y=None, height=dp(28)))
...
class StoreRegisterScreen(Screen):
    def __init__(self, **kwargs):
...
class StoreAppDetailScreen(Screen):
    ...
    def _start_download(self, item):
        ...
        # Download to the app's private cache dir.
        # FileProvider will serve it to the system installer via a
        # content:// URI — no public exposure needed.
        try:
            from jnius import autoclass
            PythonActivity = autoclass("org.kivy.android.PythonActivity")
...
class StoreAccountScreen(StoreScreenWithNav):
    nav_tab = "Account"
    def build_body(self):
        layout = BoxLayout(orientation="vertical")
        header = BoxLayout(size_hint_y=None, height=dp(56), padding=[dp(16), 0]); make_bg(header, C_SURFACE)
    ...
    def _load_profile(self):
        ...
        def _main(*_):
            ...
            signout_btn.bind(on_release=lambda _: (supabase.sign_out(), setattr(self.manager, "current", "store_login")))
...
#  APP RUNNER
# ─────────────────────────────────────────────
class SHVStoreApp(App):
    def build(self):
        Window.clearcolor = C_BG
        self.title = "SHV Store"
        self.current_app_detail = {}

        sm = ScreenManager(transition=NoTransition())
        self.sm = sm

        sm.add_widget(StoreLoginScreen(name="store_login"))
        sm.add_widget(StoreRegisterScreen(name="store_register"))
        sm.add_widget(StoreHomeScreen(name="store_home"))
        sm.add_widget(StoreCatalogScreen(name="store_catalog"))
        sm.add_widget(StoreAppDetailScreen(name="store_app_detail"))
        sm.add_widget(StoreUpdatesScreen(name="store_updates"))
        sm.add_widget(StoreAccountScreen(name="store_account"))

        sm.current = "store_login"
    ...
    def _remove_overlay(self):
        if hasattr(self, "_exit_overlay") and self._exit_overlay:
            try:
                self.root.remove_widget(self._exit_overlay)
            except Exception:
                pass
